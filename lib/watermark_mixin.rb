...
module WatermarkMixin
  ...
  def watermark_size(watermark_size)
    @watermark_size = watermark_size.nil? ? DEFAULT_WATERMARK_SIZE : watermark_size.to_f \
      if Jim::Validator.check_nil_or_greater_than_zero(watermark_size, :watermark_size) \
      && Jim::Validator.check_nil_or_between_0_and_1(watermark_size, :watermark_size)
    self
  end

  def watermark_x(watermark_x)
    @watermark_x = watermark_x.nil? ? DEFAULT_WATERMARK_X : watermark_x.to_f \
      if Jim::Validator.check_nil_or_between_0_and_1(watermark_x, :watermark_x)
    self
  end

  def watermark_y(watermark_y)
    @watermark_y = watermark_y.nil? ? DEFAULT_WATERMARK_Y : watermark_y.to_f \
      if Jim::Validator.check_nil_or_between_0_and_1(watermark_y, :watermark_y)
    self
  end
  ...
  def watermark_opacity(watermark_opacity)
    @watermark_opacity = watermark_opacity.nil? ? DEFAULT_WATERMARK_OPACITY : watermark_opacity.to_f \
      if Jim::Validator.check_nil_or_greater_than_zero(watermark_opacity, :watermark_opacity) \
      && Jim::Validator.check_nil_or_between_0_and_1(watermark_opacity, :watermark_opacity)
    self
  end
